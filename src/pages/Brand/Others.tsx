...
import NewBalanceImage from "../../assets/images/newbalance.jpg";
import BrandSection from "../../components/layouts/BrandSection";
import Column from "../../components/layouts/Column";
import Items from "../../components/layouts/Items";
import { Grid } from "@mui/material";
import { AxiosError } from "axios";
import { get_others } from "../../services/userServices";

const Others = () => {
    const [otherData, setOtherData] = useState<{ name: string, image: string }[]>([]);
    const getOtherData = async () => {
        try{
            const response = await get_others();
    ...
    return(
        <div>
            <Header />
            <BackGround sx={{ mb: 5 }}>
                <Section className="section" sx={{ background: `url(${NewBalanceImage})`, backgroundSize: 'cover', backgroundPosition: "center center" }}>
                    <BrandSection className="inner_section" sx={{ hover : "none"}}>
                        <Righteous sx={{ fontSize: "32px", color: "#FCFCFC" }}>Others</Righteous>
                    </BrandSection>
                </Section>
                <Column justifyContent="center">
                    ...
                    <Column sx={{ bgcolor: "#494949", width: "80%", maxWidth: "1140px", p: 1, borderRadius: "3px 3px 0px 0px"}}>
                        <Righteous sx={{ fontSize: "18px", color: "#FCFCFC" }}>Other Boots</Righteous>
                    </Column>
                    <Column justifyContent="center">
