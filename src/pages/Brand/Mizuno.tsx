...
import MizunoImage from "../../assets/images/mizuno.jpg";
import BrandSection from "../../components/layouts/BrandSection";
import Column from "../../components/layouts/Column";
import Items from "../../components/layouts/Items";
import { Grid } from "@mui/material";
import { AxiosError } from "axios";
import { get_mizuno } from "../../services/userServices";

const Mizuno = () => {
    const [mizunoData, setMizunoData] = useState<{ name: string, image: string }[]>([]);
    const getMizunoData = async () => {
        try{
            const response = await get_mizuno();
            if(response.status === 200){
                setMizunoData(response.data);
            }
        } catch (err: unknown) {
    ...
    return(
        <div>
            <Header />
            <BackGround sx={{ mb: 5 }}>
                <Section className="section" sx={{ background: `url(${MizunoImage})`, backgroundSize: 'cover', backgroundPosition: "center center" }}>
                    <BrandSection className="inner_section" sx={{ hover : "none"}}>
                        <Righteous sx={{ fontSize: "32px", color: "#FCFCFC" }}>Mizuno</Righteous>
                    </BrandSection>
                </Section>
